...
bool Model::load()// load model
{
	...
	while(fin >> temp_type){
			...
			else if (!strcmp(temp_type, "Conv2D"))
			{
				...
				temp = temp_i*temp_z;
				deque< int > temp_layer_size;
				temp_layer_size.push_back(temp);//input channel *ouputchannels
				...
				all_layer_size.push_back(temp_layer_size);
				all_Nue+=temp;
				cout<<" this is conv2d layer "<<setw(8)<<all_layer_type.size()-1<<" |"<<     setw(8)<<"Conv2D"<<" |"<<setw(8)<<temp_layer_size[0]<<" |"
					<<setw(8)<<temp_layer_size[1]<<" |"<<setw(8)<<temp_layer_size[2]<<" |"<<setw(8)<<temp_layer_size[3]<<" |"
			...
			else if (!strcmp( temp_type, "Dense"))
			{
				...
				temp_layer_size.push_back(temp);
				if(!strcmp( temp_actfun, "relu"))
					temp_layer_size.push_back(RELU); // 1 //o_fnReluOrPool=1
				else if(!strcmp( temp_actfun, "tanh"))
					temp_layer_size.push_back(TANH); // 2  //o_fnReluOrPool =2
				else if(!strcmp( temp_actfun, "sigmoid"))
					temp_layer_size.push_back(SIGMOID); // 3 //o_fnReluOrPool =3
				else if(!strcmp( temp_actfun, "linear"))
					temp_layer_size.push_back(LINEAR); // 0
				layernum++;
				all_layer_size.push_back(temp_layer_size);
...
float Model::generateRandomFloat() {
	//cout<<" static_cast<float>(rand()) / static_cast<float>(RAND_MAX) - 0.5f " <<static_cast<float>(rand()) / static_cast<float>(RAND_MAX) - 0.5f<<endl;
...
#ifndef   debugAllInput0
			all_data_in[input_size_ch].push_back(generateRandomFloat()); // get x number for input
#endif
#ifdef   debugAllInput0
			all_data_in[input_size_ch].push_back(generateRandomFloat()); // get x number for input
			all_data_in[input_size_ch].pop_back();//avoid changing random series. still generate  random values, but not use it.
			all_data_in[input_size_ch].push_back( 0 ); // all input zero
#endif
		}
		j++; // y + 1
		if (j == all_layer_size[0][2]) {
			input_size_ch = input_size_ch + 1;
			j = 0;
			all_data_in.push_back(deque<float>());}
	}
...
bool Model::randomweight()// load model
{
	...
	for(int i = 0; i < clayer; i++)
	{
		...
		if(all_layer_type[i] == 'c')
		{
			...
			cout<<" convlayer weightnum "<< num_w <<" len_w " <<len_w<<endl;
			check = check + num_w;
...
#ifndef  debugAllweight0

					all_weight_in[j].push_back(generateRandomFloat()); // random generated
#endif
#ifdef  debugAllweight0
					//generate all 0 weights Also， avoid changing radnom series， still generate radom values.
					all_weight_in[j].push_back(generateRandomFloat()); // random generated
				//	all_weight_in[j].push_back(0);
					all_weight_in[j].pop_back( );
					all_weight_in[j].push_back(0);
#endif
				}
				j++; // y + 1
		...
		else if(all_layer_type[i] == 'f')
		{
			...
			cout<<" fclayer weightnum "<< num_w <<" len_w " <<len_w<<endl;
			check = check + num_w;
			for (int m = 0; m < num_w; m++)
			{
				all_weight_in.push_back(deque<float>());
...
/*
	cout << "ModelCPPLoad random weight " << j << " (lines) with weight size " << check << endl;


	for (const auto &row : all_weight_in) {
		for (const auto &element : row) {
			cout <<" "<<element<<" ";
		}
		cout<<endl;
		cout<<" ModelCPPloadrandomweight_row.size "<<row.size()<<" "<<endl;
